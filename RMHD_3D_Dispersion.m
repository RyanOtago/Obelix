function [omegaout] = RMHD_3D_Dispersion(kn, LX, NX, bpar, va, nu, TF, no_perp, no_alfven, strengthpx, strengthmx, strengthpy, strengthmy)
...
for strengthnn = 1:length(strengthpx)
        ...
        for meshnn = 1:length(NX)
            for knnn = 1:length(kn)
                ...
                for i = (1:length(time))
                    ...
                    zp_x  = strengthpx(strengthnn);%real(ifftn(KX.*zeta_plus));
                    zm_x  = strengthmx(strengthnn);%real(ifftn(KX.*zeta_minus));
                    zp_y  = strengthpy(strengthnn);%real(ifftn(KY.*zeta_plus));
                    zm_y  = strengthmy(strengthnn);%real(ifftn(KY.*zeta_minus));
    ...
    disp(['Just finished strength = ' num2str(strength(strengthnn))])
end
omegaout = w;
